...
class Solution {
private:
    bool helper(string value, int k) { // pattern不为空，value不为空. 判断是否可以平均k次切分value
        int m = value.size();
...
public:
    ...
    bool patternMatching(string pattern, string value) {
        ...
        // 2.2.2 a,b都不为空; 枚举a, b匹配的长度，使得 a个数 * len_a + b个数 * len_b = m; len_a唯一确定len_b，只需枚举len_a
